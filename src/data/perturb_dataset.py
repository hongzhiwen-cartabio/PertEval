...
class PerturbData(Dataset):
    ...
    def __init__(self, adata, data_path, spectral_parameter, spectra_params, fm, stage, **kwargs):
        ...
        # Data loading logic
        if self.data_name in ["norman_1", "norman_2", "replogle_k562"]:
        ...
        elif "replogle" in self.data_name:
            data_file = f"{self.data_path}/input_features/train_data.pkl.gz"
            if not os.path.exists(data_file):
                ctrl_adata, pert_adata, pert_list = self.preprocess(adata)
                pp_data = self.featurise_replogle(pert_adata, pert_list, ctrl_adata)
                self.X_train, self.train_target, self.X_test, self.test_target = pp_data
            else:
                self._load_preprocessed_data(feature_path)
    ...
    # 3.2 构建输入特征：ctrl_embedding + pert_embedding_mean
        ...
        #     X_train.append(np.concatenate([ctrl_emb, pert_emb]))  # 拼接为输入特征
    ...
    #         if self.data_name == "norman_1":
    #             single_gene_mask = ["," not in name for name in adata.obs['condition']]
    ...
    #     self.genes = adata.var['gene_symbols'].tolist() if "norman" in self.data_name else adata.var.index.tolist()
    ...
    #     #ipdb.set_trace()
    #     # New dataset splitting (80/20)
    #     pert_list = list(adata.obs['condition'].unique())
    #     ipdb.set_trace()
    #     pert_list.remove('ctrl')
    #     train_perts, test_perts = train_test_split(pert_list, test_size=0.2, random_state=42)
    ...
    #          # 检查 fm_pert_data 是否包含所有扰动
    #         missing_perts = [pert for pert in unique_perts if pert not in fm_pert_data]
    #         if missing_perts:
    #             print(f"Warning: The following perturbations are missing in fm_pert_data: {missing_perts}")
    ...
    #     with gzip.open(f"{self.data_path}/input_features/{self.fm}/train_data.pkl.gz", "wb") as f:
    #         pkl.dump((X_train, train_target), f)
    #     with gzip.open(f"{self.data_path}/input_features/{self.fm}/val_data.pkl.gz", "wb") as f:
    #         pkl.dump((X_val, val_target), f)
    #     with gzip.open(f"{self.data_path}/input_features/{self.fm}/test_data.pkl.gz", "wb") as f:
    #         pkl.dump((X_test, test_target), f)
